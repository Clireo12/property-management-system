            ...
            <main class="col-lg-10 col-md-9 ms-sm-auto px-md-4 py-4">
                    ...
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb mb-0"
                            ...
                            <li class="breadcrumb-item active"
                                style="color: #f8f9fa; font-size: 16px">报修管理</li>
                        </ol>
                    </nav>
                </div>
                ...
                <div class="card shadow-sm mb-4">
                    <div class="card-header bg-primary text-white">
                        <h5 class="mb-0">
                            <i class="fas fa-tools me-2"></i>报修管理
                        </h5>
                    </div>
                    <div class="card-body">
                        <a class="btn btn-success mb-3" href="maintain/maintain-add.jsp">
                            <i class="fas fa-plus me-2"></i>添加记录
                        </a>
                        
                        <!-- 搜索表单 -->
                        <form action="main" method="get" class="search-container">
                            <input type="hidden" name="action" value="maintainSearch">
                            <input type="text" class="form-control" id="keyword"
                                name="keyword" placeholder="输入报修物品、房号或状态" 
                                value="${param.keyword}">
                            <button type="submit" class="btn btn-primary me-2">
                                <i class="fas fa-search me-2"></i>搜索
                            </button>
                            <a href="main?action=maintainList" class="btn btn-secondary">
                        ...
                        <div class="table-responsive">
                            <c:choose>
                                <c:when test="${empty maintains}">
                                    <div class="alert alert-warning text-center my-4">
                                        <i class="fas fa-exclamation-circle me-2"></i> 未找到报修记录
                                        <c:if test="${not empty param.keyword}">
                                            (搜索关键词: "${param.keyword}")
                                            <br>
                                            <a href="main?action=maintainList" class="btn btn-sm btn-outline-primary mt-2">
                                ...
                                <c:otherwise>
                                    <c:if test="${not empty param.keyword}">
                                        <div class="alert alert-info">
                                            ...
                                            <span class="badge bg-primary" style="color:black">${fn:length(maintains)}</span> 条记录
                                            (关键词: "${param.keyword}")
                                        </div>
                                    </c:if>
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="action-buttons">
                                                        <a class="btn btn-info"
                                                            href="main?action=findById&id=${maintain.id}"> 
                                                            <i class="fas fa-edit me-2"></i>编辑
                                                        </a> 
                                                        <a class="btn btn-danger"
                                                            href="javascript:if(confirm('此条记录将被永久删除，确认删除吗？')) window.location.href='main?action=maintainDelete&id=${maintain.id}';">
