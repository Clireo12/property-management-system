                    ...
                    <div class="card-body">
                        <div class="search-container">
                            <a class="btn btn-success"
                                href="maintain/user-maintain-add.jsp"> 
                                <i class="fas fa-plus me-2"></i>报修
                            </a>
                            
                            <!-- 搜索表单 -->
                            <form action="main" method="get" class="search-form">
                                <input type="hidden" name="action" value="maintainUserSearch">
                                <input type="hidden" name="username" value="${customAccount.username}">
                                <input type="text" class="form-control me-2" id="keyword"
                                    name="keyword" placeholder="输入报修物品、状态或房门号" 
                                    value="${param.keyword}">
                                ...
                                <c:when test="${empty maintains}">
                                    <div class="alert alert-warning text-center my-4">
                                        ...
                                        <c:if test="${not empty param.keyword}">
                                            (搜索关键词: "${param.keyword}")
                                            <br>
                                            <a href="main?action=maintainUserList&username=${customAccount.username}" 
                                ...
                                <c:otherwise>
                                    <c:if test="${not empty param.keyword}">
                                        <div class="alert alert-info mb-3 py-2">
                                            ...
                                            搜索到 <span class="badge bg-primary">${fn:length(maintains)}</span> 条记录
                                            (关键词: "${param.keyword}")
                                        </div>
                                    </c:if>
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="action-buttons">
                                                        <a class="btn btn-danger"
                                                            href="javascript:if(confirm('确定要删除这条报修记录吗？')) 
                                                            window.location.href='main?action=maintainUserDelete&id=${maintain.id}&username=${customAccount.username}';">
